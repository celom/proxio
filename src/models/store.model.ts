 ...
 */
export type SourceParams<T extends RecordMap, K extends keyof T> = Parameters<T[K]>[0];
/**
 * The return type of a specific method in a source map.
 * @template T - The source map.
 * @template K - The key of the method in the source map.
 */
export type SourceReturnType<T extends RecordMap, K extends keyof T> = ReturnType<T[K]>;
 ...
 * @template K - The key of the source map.
 */
export type StoreReturnPromise<T extends RecordMap, K extends keyof T> = { data: Awaited<SourceReturnType<T, K>> };
/**
 * A loadable value of a specific source method.
 * @template T - The source map.
 * @template K - The key of the source map.
 */
export type StoreReturnLoadable<T extends RecordMap, K extends keyof T> = Loadable<SourceReturnType<T, K>>;
/**
 * A store option object that holds the suspense and delay properties.
 */
export type StoreOptions<S extends boolean> = {
    force?: boolean;
    /**
 ...
 */
export type StorePropMap<S extends RecordMap> = {
    source: SourceMap<S>;
    props: PropMap<S>;
};
/**
 ...
 */
export type Store<T extends RecordMap, K extends keyof T, S extends boolean> = S extends true
    ? StoreReturnPromise<T, K>
    : StoreReturnLoadable<T, K>;